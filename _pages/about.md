---
permalink: /
title: "Juntian Zhang"
author_profile: true
redirect_from:
  - /about/
  - /about.html
---

I am currently a <strong>Master's student</strong> in Artificial Intelligence at the Gaoling School of Artificial Intelligence, Renmin University of China.

Before that, I received my undergraduate education at Chu Kochen Honors College, Zhejiang University, majoring in Computer Science and Technology.

<p><strong>Research interests:</strong> <span class="hl-interest">Natural Language Processing (NLP)</span>, <span class="hl-interest">Large Language Models (LLMs)</span>, and <span class="hl-interest">Multimodal Large Language Models (MLLMs)</span>.</p>

<style>
  .section-title {
    position: relative;
    display: block;
    margin: 1.75rem 0 0.95rem;
    padding-bottom: 0.45rem;
    border-bottom: 1px solid #e7e9ee;
    line-height: 1.25;
    font-size: 1.42rem;
    font-weight: 700;
    color: #0f172a;
    letter-spacing: 0.01em;
  }
  .section-title::after {
    content: "";
    position: absolute;
    left: 0;
    bottom: -1px;
    width: 4.4rem;
    height: 3px;
    border-radius: 999px;
    background: linear-gradient(90deg, #0f766e, #14b8a6);
  }
  .hl-school {
    color: #0f766e;
    font-weight: 600;
  }
  .hl-interest {
    color: #1e3a8a;
    font-weight: 600;
  }
  .profile-logo-list {
    margin: 0.75rem 0 1.25rem;
    display: grid;
    gap: 0.8rem;
  }
  .profile-logo-item {
    display: grid;
    grid-template-columns: 48px 1fr;
    gap: 0.75rem;
    align-items: center;
  }
  .profile-logo-item img {
    width: 42px;
    height: 42px;
    object-fit: contain;
    border-radius: 6px;
    border: 1px solid #e8e8e8;
    background: #fff;
    padding: 3px;
  }
  .edu-logo-list .profile-logo-item img {
    width: 56px;
    height: 56px;
  }
  .profile-logo-item-title {
    margin: 0;
    font-weight: 600;
    line-height: 1.3;
  }
  .profile-logo-item-sub {
    margin: 0.15rem 0 0;
    color: #4f4f4f;
    font-size: 0.95rem;
    line-height: 1.35;
  }
  .reward-list {
    margin: 0.65rem 0 1.3rem;
    padding-left: 1.25rem;
  }
  .reward-list li {
    margin: 0.32rem 0;
    line-height: 1.45;
    color: #374151;
  }
</style>

<h2 id="education" class="section-title">Education</h2>

<div class="profile-logo-list edu-logo-list">
  <div class="profile-logo-item">
    <img src="{{ '/images/logos/ruc-logo-s.png' | relative_url }}" alt="Renmin University of China logo">
    <div>
      <p class="profile-logo-item-title"><span class="hl-school">Renmin University of China</span>, <span class="hl-school">Gaoling School of Artificial Intelligence</span></p>
      <p class="profile-logo-item-sub">Major: Artificial Intelligence (Master)</p>
    </div>
  </div>
  <div class="profile-logo-item">
    <img src="{{ '/images/logos/zju-icon.png' | relative_url }}" alt="Zhejiang University logo">
    <div>
      <p class="profile-logo-item-title"><span class="hl-school">Zhejiang University</span>, <span class="hl-school">Chu Kochen Honors College</span></p>
      <p class="profile-logo-item-sub">Major: Computer Science and Technology (Undergraduate)</p>
    </div>
  </div>
</div>

<h2 id="publications" class="section-title">Publications</h2>

{% include publications-inline.html %}

<h2 id="internships" class="section-title">Internships</h2>

<div class="profile-logo-list">
  <div class="profile-logo-item">
    <img src="{{ '/images/logos/meituan.svg' | relative_url }}" alt="Meituan logo">
    <div>
      <p class="profile-logo-item-title">Meituan</p>
      <p class="profile-logo-item-sub">Large Model Algorithm Research Intern.</p>
    </div>
  </div>
  <div class="profile-logo-item">
    <img src="{{ '/images/logos/wechat.svg' | relative_url }}" alt="Tencent WeChat logo">
    <div>
      <p class="profile-logo-item-title">Tencent WeChat</p>
      <p class="profile-logo-item-sub">Large Model Application Center Intern.</p>
    </div>
  </div>
  <div class="profile-logo-item">
    <img src="{{ '/images/logos/bytedance.svg' | relative_url }}" alt="ByteDance TikTok logo">
    <div>
      <p class="profile-logo-item-title">ByteDance TikTok</p>
      <p class="profile-logo-item-sub">Frontier Technology Talent Program Intern.</p>
    </div>
  </div>
</div>

<h2 id="awards" class="section-title">Awards</h2>

<ul class="reward-list">
  <li>Selected awards and honors are being organized and will be updated here.</li>
</ul>

<h2 id="links" class="section-title">Links</h2>

- Google Scholar: [https://scholar.google.com/citations?user=K-6vOfkAAAAJ&hl=en](https://scholar.google.com/citations?user=K-6vOfkAAAAJ&hl=en)
- GitHub: [https://github.com/JuntianZhang1216](https://github.com/JuntianZhang1216)
